...
// Satırları uygulama nesnesine çeviren yardımcılar (snake_case -> camelCase)
const toUser = (r) => r && ({ id: r.id, phone: r.phone, name: r.name,
  tcVerified: r.tc_verified, iban: r.iban, address: r.address,
  cardToken: r.card_token, createdAt: r.created_at });
const toGroup = (r) => r && ({ id: r.id, name: r.name, ownerId: r.owner_id,
  memberCount: r.member_count, unit: r.unit, amount: Number(r.amount),
  period: r.period, startDate: r.start_date, orderMethod: r.order_method,
  inviteCode: r.invite_code, status: r.status, createdAt: r.created_at });
const toMember = (r) => r && ({ id: r.id, groupId: r.group_id, userId: r.user_id,
  slotNo: r.slot_no, role: r.role, joinDate: r.join_date });
const toRound = (r) => r && ({ id: r.id, groupId: r.group_id, roundNo: r.round_no,
  beneficiaryId: r.beneficiary_id, dueDate: r.due_date, status: r.status });
const toPayment = (r) => r && ({ id: r.id, roundId: r.round_id, payerId: r.payer_id,
  amount: Number(r.amount), method: r.method, status: r.status,
  providerRef: r.provider_ref, paidAt: r.paid_at });
...
export const postgresStore = {
  ...
  async createUser(u) {
    await query(
      `INSERT INTO users (id, phone, name, tc_verified, iban, address, card_token)
  ...
  async createGroup(g) {
    await query(
      `INSERT INTO groups (id, name, owner_id, member_count, unit, amount, period, start_date, order_method, invite_code, status)
  ...
  async listUserGroups(userId) {
    const { rows } = await query(
      `SELECT g.* FROM groups g JOIN memberships m ON m.group_id=g.id WHERE m.user_id=$1 ORDER BY g.created_at DESC`,
      [userId]);
  ...
  async addMembership(x) {
    await query(
      `INSERT INTO memberships (id, group_id, user_id, slot_no, role) VALUES ($1,$2,$3,$4,$5)`,
      [x.id, x.groupId, x.userId, x.slotNo, x.role]);
    return x;
  },
  async membershipsOfGroup(groupId) {
    const { rows } = await query('SELECT * FROM memberships WHERE group_id=$1', [groupId]);
  ...
  async createRound(r) {
    await query(
      `INSERT INTO rounds (id, group_id, round_no, beneficiary_id, due_date, status) VALUES ($1,$2,$3,$4,$5,$6)`,
  ...
  async roundsOfGroup(groupId) {
    const { rows } = await query('SELECT * FROM rounds WHERE group_id=$1 ORDER BY round_no', [groupId]);
  ...
  async createPayment(p) {
    await query(
      `INSERT INTO payments (id, round_id, payer_id, amount, method, status, provider_ref, paid_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [p.id, p.roundId, p.payerId, p.amount, p.method, p.status, p.providerRef, p.paidAt]);
    return p;
  },
  async paymentsOfRound(roundId) {
    const { rows } = await query('SELECT * FROM payments WHERE round_id=$1', [roundId]);
  ...
  async createGoldOrder(o) {
    await query(
      `INSERT INTO gold_orders (id, round_id, beneficiary_id, gram, provider_ref, shipping_status, tracking_no)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
  ...
  async getOtp(phone) {
    const { rows } = await query('SELECT * FROM otps WHERE phone=$1', [phone]);
    return rows[0] ? { code: rows[0].code, expiresAt: Number(rows[0].expires_at) } : null;
  },
  ...
  async appendAudit(entity, action, actorId, data = {}) {
    ...
    await query(
      `INSERT INTO audit_log (entity, action, actor_id, data, ts, prev_hash, hash)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [entity, action, actorId, JSON.stringify(data), ts, prevHash, hash]);
    return { entity, action, actorId, data, ts, prevHash, hash };
  },
  async verifyAuditChain() {
    const { rows } = await query('SELECT id FROM audit_log');
